...
public class NoticeController {
    ...
    @RequestMapping("/FINFO/detail")
    public String FINFO_DETAIL(@RequestParam("f_code") int f_code, Model model){
        notice field = noticeService.getFieldDetail(f_code);
        model.addAttribute("field", field);
        if (field != null) {
            String mapQuery = URLEncoder.encode(field.getF_NAME() + " " + field.getF_ADD(), StandardCharsets.UTF_8);
            model.addAttribute("naverMapUrl", "https://map.naver.com/p/search/" + mapQuery);
            model.addAttribute("kakaoMapUrl", "https://map.kakao.com/link/search/" + mapQuery);
            model.addAttribute("googleMapUrl", "https://www.google.com/maps/search/?api=1&query=" + mapQuery);
        }
        return "Page/Notice/FINFO/FDETAIL";
    }
